...
import { labelsProperty, modelRLC } from '../descriptions';
...
export async function execute(this: IExecuteFunctions, i: number): Promise<INodeExecutionData[]> {
	...
	const labelsRaw = this.getNodeParameter('labels.values', i, []) as Array<{
		key: string;
		value: string;
	}>;
	...
	const text = `Generate a transcript of the speech${
		options.startTime ? ` from ${options.startTime}` : ''
	}${options.endTime ? ` to ${options.endTime}` : ''}`;
